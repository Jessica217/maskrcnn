...
def get_file_pos(folder_path):
    ...
    for json_file in json_files:
        ...
        img = np.zeros((points["imageHeight"], points["imageWidth"], 3), dtype = np.uint8)# 使用cv2创建与原图尺寸相同的图像
        pts1 = np.array(left_kidney_pos, np.int32)
        pts2 = np.array(right_kidney_pos, np.int32)
        cv2.fillPoly(img, [pts1], color = (255, 255, 255))
        cv2.fillPoly(img, [pts2], color = (255, 255, 255))
        # print(img)
